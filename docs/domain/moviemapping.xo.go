...
func (mm *MovieMapping) Insert(db XODB) error {
	...
	// run query
	XOLog(sqlstr, mm.Title, mm.OriginalTitle, mm.ReleaseDate, mm.VoteAverage, mm.PosterPath)
	err = db.QueryRow(sqlstr, mm.Title, mm.OriginalTitle, mm.ReleaseDate, mm.VoteAverage, mm.PosterPath).Scan(&mm.ID)
	if err != nil {
		return err
	}
